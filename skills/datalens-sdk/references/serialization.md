...
Every fetched entity carries `response_snapshot` — an owned snapshot of the JSON object the server returned. `to_file()` writes it to disk; `client.raw` consumes it (in memory or from disk). Editor is the narrow safety exception: API v3 read-only `entry.data.secrets` is removed before the snapshot reaches domain state, artifacts, or raw mutation payloads. Two consequences:
