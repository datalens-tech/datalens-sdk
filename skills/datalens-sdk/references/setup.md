...
The package is pinned to **`datalens-sdk==0.1.0`**. Preflight verifies the pin and emits the exact reinstall command on drift — never install or upgrade unpinned.
...
| `ACTUAL` present but not `0.1.0` (reported as `SDK=needs_install`) | `needs_input` | Version drift. Run `INSTALL_CMD` — it reinstalls the pinned `datalens-sdk==0.1.0`. |
