...
Factory: `client.create.wizard_chart.pie(name=..., location=...)`
`chart.visualization_id`: `pie`
...
| --- | --- | --- | --- | --- |
| `x()` | `dimensions` | slice category | no | 1 |
| semantic color API | `colors` | slice color | no | 1 |
| `y()` | `measures` | slice size | yes | 1 |
...
| --- | --- | --- |
| `client.create.wizard_chart.pie()` | `name: str`, `location: EntryLocation` | C |
...
chart = (
    client.create.wizard_chart.pie(
        name="Pie",
...
chart = chart.update.measures([profit]).measure_format(profit, format="currency").mode("publish").execute()
```

## Constraints and gotchas

- `x` aliases optional `dimensions`; `y` aliases required `measures`; both have capacity 1.
- Without explicit `color_by_dimension()`, the SDK uses the dimension as the color source.
- Only dimension color is supported; measure-gradient and measure-name color are not.
