...
Factory: `client.create.wizard_chart.column(name=..., location=...)`
`chart.visualization_id`: `column`
...
| --- | --- | --- |
| `client.create.wizard_chart.column()` | `name: str`, `location: EntryLocation` | C |
...
chart = chart.update.segments([new_segment]).color_by_dimension(new_segment).mode("publish").execute()
```

## Constraints and gotchas

- `x` is optional with capacity 2; `y` carries measures.
- Multiple measures insert `Measure Names` into `x` and consume one of its two
  category slots. Use at most one ordinary `x` category with multi-measure
  coloring.
- Dimension, measure-gradient, and measure-name color encodings are all supported.
- `segments()` is supported and is distinct from `color_by_dimension()`.
- For multiple measures, use `color_by_measure_name(colors_map={...})` only
  after placing at least two measures.
- Supported transition: `column -> line`.
