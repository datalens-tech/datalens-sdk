# 100% stacked column Wizard chart

Factory: `client.create.wizard_chart.column_100p(name=..., location=...)`
`chart.visualization_id`: `column100p`
...
| --- | --- | --- |
| `client.create.wizard_chart.column_100p()` | `name: str`, `location: EntryLocation` | C |
...
chart = (
    client.create.wizard_chart.column_100p(
...
- Canonical viz id is `column100p`, not the factory spelling `column_100p`.
- `x` has capacity 2; `y` and the color split each have capacity 1.
- `segments()` is supported; measure-gradient and measure-name color encodings are not.
